...
void scramble(void);
void transpose(char grid[DIMENSION][DIMENSION]);
void swapCol(char grid[DIMENSION][DIMENSION]);
...
void draw(void)
{
    // DONE
    printf("\n");
    for  (int x = 0; x < DIMENSION; x++) //iterate over row
    {
        for  (int y = 0; y < DIMENSION; y++) //iterate over column
        {
...
/**
 * Returns true if word is found in grid.
 */
bool find(string s)
{
...
bool lookup(string s)
{
    ...
    {
    if(strcmp(s, dictionary.words[f].letters) == 0)
    {
    if(dictionary.words[f].found == 0)
    {
    dictionary.words[f].found = true;
    return true;
    }
    }
...
//transposes rows in matrix....used in scramble function
void transpose(char grid[DIMENSION][DIMENSION])
{
    int i=0;
    int j=0;
...
// swaps colums in matrix, used in scramble function
void swapCol(char grid[DIMENSION][DIMENSION])
{
    int i=0;
    int j=0;
